...
{%- for attr in entity.attributes %}
    {{ attr.id }} {% if attr.datatype == 'Integer' %}INTEGER{% elif attr.datatype == 'Date' %}DATE{% elif attr.datatype == 'Boolean' %}BOOLEAN{% elif attr.datatype == 'Decimal' %}DECIMAL{% else %}VARCHAR{% endif %}{%- if not loop.last or entity.identifiers | length > 0 %},{% endif %}
{%- endfor %}
